...
router.post("/register", async (req,res) => {
    try {
        ...
        const checkUser = await User.findOne({ username }).populate('friends requests following').exec();;
...
router.post("/login", async (req,res) => {
    ...
    const user = await User.findOne({ username }).populate('friends requests following').exec();;
